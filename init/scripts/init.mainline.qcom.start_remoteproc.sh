...
cd /sys/class/remoteproc || (print_log "Failed to enter /sys/class/remoteproc"; exit 1)

START_SINGLE_REMOTEPROC=$1
if [ "$START_SINGLE_REMOTEPROC" ]; then
    START_SINGLE_REMOTEPROC_DONE=0
    print_log "Start remoteproc ${START_SINGLE_REMOTEPROC} on request"
fi
...
for rproc in `ls`; do
    ...
    if [ "$rproc_prop_firmware" ]; then
        print_log "Set remoteproc ${rproc_name} firmware to: ${rproc_prop_firmware}"
        echo "${rproc_prop_firmware}" > ${rproc}/firmware
    fi

    print_log "Triggering start of remoteproc ${rproc_name}"
    echo "start" > ${rproc}/state
    start_ret=$?
    print_log "Finished triggering start of remoteproc ${rproc_name}, return code is ${start_ret}"
...
if [ "$START_SINGLE_REMOTEPROC" ] && [ "$START_SINGLE_REMOTEPROC_DONE" != "1" ]; then
    print_log "Unable to find the remoteproc that was requested to start: $START_SINGLE_REMOTEPROC"
fi
