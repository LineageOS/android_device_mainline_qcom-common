...
$(warning Target device prefers using modules from mainline/qcom-common-ext repository, however it is not available. Featu
res may not work as intended.)
endif
endif
...
ifeq ($(TARGET_QCOM_SOC_FAMILY),)
    ifeq ($(TARGET_QCOM_SOC),)
        ...
        else ifneq ($(filter msm8956 msm8976%,$(TARGET_QCOM_SOC)),)
            TARGET_QCOM_SOC_FAMILY := msm8956
        else ifneq ($(filter msm8998,$(TARGET_QCOM_SOC)),)
            TARGET_QCOM_SOC_FAMILY := msm8998
        else ifneq ($(filter sdm670 sdm710,$(TARGET_QCOM_SOC)),)
        ...
        else ifneq ($(filter cq8725s cq8750s,$(TARGET_QCOM_SOC)),)
            TARGET_QCOM_SOC_FAMILY := sm8750
        else
